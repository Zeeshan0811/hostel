...
if (isset($_POST['update'])) {
	...
	if (isset($fileName)) {
		$query = "update  userRegistration set regNo=?,firstName=?,middleName=?,lastName=?,gender=?,contactNo=?,updationDate=?,profile_picture=? where id=?";
		$stmt = $mysqli->prepare($query);
		$rc = $stmt->bind_param('sssssissi', $regno, $fname, $mname, $lname, $gender, $contactno, $udate, $fileName, $aid);
	} else {
											...
											<form method="post" action="" name="registration" class="form-horizontal" onSubmit="return valid();" enctype="multipart/form-data">
												<?php if (!empty($row->profile_picture)) {  ?>
													<div class="col-md-4 text-center">
														<img src="<?php echo './uploads/' . $row->profile_picture; ?>" alt="" width="200px">
													</div>
												<?php } ?>
												<div class="col-md-8">
													...
													<div class="form-group">
														<label class="col-sm-2 control-label">Profie Picture: </label>
														<div class="col-sm-8">
															<input type="file" name="profile_picture" id="profile_picture" class="form-control" value="<?php echo $row->profile_picture; ?>" readonly>
														</div>
													</div>
												</div>
											<?php } ?>
