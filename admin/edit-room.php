...
if ($_POST['submit']) {
	...
	$fees = $_POST['fees'];
	$id = $_GET['id'];
	$query = "update rooms set hall_id=?,block_id=?,seater=?,fees=? where id=?";
	$stmt = $mysqli->prepare($query);
	$rc = $stmt->bind_param('iiiii', $hall_id, $block_id, $seater, $fees, $id);
	$stmt->execute();
												...
												<div class="form-group">
													...
													<div class="col-sm-8">
														<Select name="hall_id" class="form-control" required>
															...
															?>
																<option value="<?php echo $halls->id; ?>" <?php if ($row->hall_id == $halls->id) echo 'selected'; ?>><?php echo $halls->hall_name; ?></option>
															<?php } ?>
														</Select>
													</div>
												</div>
												<div class="form-group">
													...
													<div class="col-sm-8">
														<Select name="block_id" class="form-control" required>
															...
															?>
																<option value="<?php echo $blocks->id; ?>" <?php if ($row->block_id == $blocks->id) echo 'selected'; ?>>
																	<?php echo $blocks->block_name . " (" . $blocks->block_type . ")"; ?>
																</option>
															<?php } ?>
														</Select>
													</div>
